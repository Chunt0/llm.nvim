...
describe("Memory.build_messages", function()
  ...
  it("interleaves history between system and the new user message", function()
    ...
    assert.are.equal("system", msgs[1].role)
    assert.are.equal("user", msgs[2].role)
    assert.are.equal("q1", msgs[2].content)
    assert.are.equal("assistant", msgs[3].role)
    assert.are.equal("a1", msgs[3].content)
    assert.are.equal("user", msgs[4].role)
    assert.are.equal("q2", msgs[4].content)
  end)
  ...
  it("the new user prompt is always last", function()
    ...
    Memory.append(BUF, "assistant", "reply")
  ...
  -- P3 regression: a dangling trailing user turn (left behind if a request
  -- failed before the assistant replied) must be merged into the new prompt,
  -- never sent as two consecutive user messages — Anthropic rejects
  -- non-alternating roles, which used to wedge the chat until :LLMClear.
  it("merges a dangling trailing user turn into the new prompt", function()
    Memory.append(BUF, "user", "old")
    local msgs = Memory.build_messages(BUF, "SYS", "new question")
    assert.are.equal(2, #msgs) -- system + single merged user message
    assert.are.equal("user", msgs[#msgs].role)
    assert.are.equal("old\n\nnew question", msgs[#msgs].content)
  end)

  it("merges consecutive same-role history entries so roles always alternate", function()
    Memory.append(BUF, "user", "u1")
    Memory.append(BUF, "user", "u2")
    Memory.append(BUF, "assistant", "a1")
    Memory.append(BUF, "assistant", "a2")
    local msgs = Memory.build_messages(BUF, "SYS", "next")
    -- Expected: [system, user:u1+u2, assistant:a1+a2, user:next]
    assert.are.equal(4, #msgs)
    assert.are.equal("u1\n\nu2", msgs[2].content)
    assert.are.equal("a1\n\na2", msgs[3].content)
    local prev_role = nil
    for _, m in ipairs(msgs) do
      if m.role ~= "system" then
        assert.are_not.equal(prev_role, m.role, "roles must alternate")
        prev_role = m.role
      end
    end
  end)
