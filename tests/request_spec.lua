...
describe("make_openai_spec_curl_args", function()
  ...
  it("DIAGNOSTIC: prints model name and endpoint from constants", function()
    print(string.format(
      "\n  [DIAGNOSTIC] constants.models.openai        = %q",
      tostring(constants.models.openai)
    ))
    print(string.format(
      "  [DIAGNOSTIC] constants.api_endpoints.openai = %q",
      tostring(constants.api_endpoints.openai)
    ))
    assert.is_string(constants.models.openai,
      "constants.models.openai must be a string")
    assert.is_true(#constants.models.openai > 0,
      "constants.models.openai must not be empty")
  end)

  it("returns a table with 'args' (table) and 'config' (string)", function()
    local r = llm.make_openai_spec_curl_args(
      { model = "gpt-4o", url = "", api_key_name = nil },
      "hello", ""
    )
  ...
  it("config targets the OpenAI Responses API endpoint by default", function()
    ...
    assert.is_truthy(r.config:find("api.openai.com/v1/responses"),
      "config should contain the Responses API URL")
  end)

  it("config uses a custom url when opts.url is set", function()
    local r = llm.make_openai_spec_curl_args(
      { model = "gpt-4o", url = "https://proxy.example.com/v1/responses" },
      "hello", ""
    )
    assert.is_truthy(r.config:find("proxy.example.com", 1, true),
      "config should use the custom URL")
    assert.is_falsy(r.config:find("api.openai.com", 1, true),
      "default URL should not appear when custom URL is given")
  end)
  ...
  it("JSON body uses the model from constants for a real invocation", function()
    ...
    assert.are.equal(constants.models.openai, body.model,
      string.format(
    ...
    print(string.format(
      "\n  [DIAGNOSTIC] Actual model sent in request body: %q",
      tostring(body.model)
    ))
  end)
  ...
  it("JSON body has instructions set from system_prompt on first call", function()
    ...
    assert.are.equal("MySysPrompt", body.instructions,
      "instructions should equal the system_prompt")
  end)
  ...
  it("JSON body defaults reasoning.effort to 'minimal' when not specified", function()
    local r    = llm.make_openai_spec_curl_args(
      { model = "gpt-4o", url = "", api_key_name = nil },
      "q", ""
    )
    ...
    assert.are.equal("minimal", body.reasoning.effort,
      "default reasoning.effort should be 'minimal'")
  ...
  it("JSON body sets instructions=nil when opts.messages is provided", function()
    ...
    assert.is_nil(body.instructions,
      "instructions must be nil when opts.messages is provided")
    assert.is_table(body.input,
      "input must be the messages table when opts.messages is set")
  end)
  ...
  it("first call has no previous_response_id (openai_count == 0)", function()
    ...
    assert.is_nil(body.previous_response_id,
      "no previous_response_id on first call")
  end)
end)
...
describe("make_anthropic_spec_curl_args", function()
  it("DIAGNOSTIC: prints model name and endpoint from constants", function()
    print(string.format(
      "\n  [DIAGNOSTIC] constants.models.anthropic        = %q",
  ...
  it("config targets the Anthropic Messages API endpoint", function()
    ...
    assert.is_truthy(r.config:find("api.anthropic.com/v1/messages"),
      "config should target the Anthropic Messages API")
  end)
  ...
  it("config contains the anthropic-version header", function()
    ...
    assert.is_truthy(r.config:find("anthropic-version", 1, true),
  ...
  it("split_system moves system role into body.system and removes it from messages", function()
    ...
    local body = extract_body(r.config)
    assert.is_table(body)
    assert.are.equal("Expert system prompt", body.system,
      "system message should be extracted to body.system")
    for _, m in ipairs(body.messages or {}) do
      assert.are_not.equal("system", m.role,
        "system role should not remain in messages array")
    end
  end)
  ...
  it("includes a user message in the messages array", function()
    ...
    for _, m in ipairs(body.messages or {}) do
      if m.role == "user" then has_user = true; break end
    end
...
describe("make_ollama_spec_curl_args", function()
  it("DIAGNOSTIC: prints model name and endpoint from constants", function()
    print(string.format(
      "\n  [DIAGNOSTIC] constants.models.ollama        = %q",
      tostring(constants.models.ollama)
    ))
    print(string.format(
      "  [DIAGNOSTIC] constants.api_endpoints.ollama = %q",
  ...
  it("config targets the Ollama Chat API endpoint", function()
    ...
    assert.is_truthy(r.config:find("ollama", 1, true),
      "config should mention ollama in the URL")
  end)
  ...
  it("config does NOT contain an Authorization header (Ollama is unauthenticated)", function()
    ...
    assert.is_falsy(r.config:find("Authorization", 1, true),
  ...
  it("messages include a system entry when system_prompt is provided", function()
    local r    = llm.make_ollama_spec_curl_args(
      { model = constants.models.ollama, url = "" },
      "user query", "sys prompt"
    )
    local body = extract_body(r.config)
    assert.is_table(body)
    local has_system = false
    for _, m in ipairs(body.messages or {}) do
      if m.role == "system" then has_system = true; break end
    end
    assert.is_true(has_system,
      "messages should contain a system entry when system_prompt is given")
  end)

  it("messages do NOT include a system entry when system_prompt is empty", function()
    local r    = llm.make_ollama_spec_curl_args(
      { model = constants.models.ollama, url = "" },
      "user query", ""       -- empty system prompt
    )
    local body = extract_body(r.config)
    assert.is_table(body)
    for _, m in ipairs(body.messages or {}) do
      assert.are_not.equal("system", m.role,
        "no system message should appear when system_prompt is empty")
    end
  end)

  it("uses the custom URL from opts.url when provided", function()
    local r = llm.make_ollama_spec_curl_args(
      { model = "phi3", url = "http://localhost:11434/api/chat" },
      "hello", ""
    )
    assert.is_truthy(r.config:find("localhost:11434", 1, true))
    assert.is_falsy(r.config:find("putty-ai.com",    1, true),
      "default URL should not appear when a custom URL is given")
  end)
  ...
  it("uses opts.messages directly when provided", function()
    ...
    local body = extract_body(r.config)
    assert.is_table(body)
    assert.are.equal(#msgs, #body.messages,
...
describe("make_anthropic_spec_curl_args — multi-turn history", function()
  ...
  before_each(function() Memory.clear(BUF) end)
  ...
  it("single-turn: body has exactly one user message", function()
    ...
    local user_count = 0
    for _, m in ipairs(body.messages or {}) do
      if m.role == "user" then user_count = user_count + 1 end
    end
  ...
  it("system prompt is promoted to body.system even with history", function()
    ...
    for _, m in ipairs(body.messages or {}) do
      assert.are_not.equal("system", m.role,
        "system role must not remain in messages array")
    end
  end)
end)
...
describe("make_ollama_spec_curl_args — multi-turn history", function()
  ...
  it("two-turn: body retains the full conversation in order", function()
    ...
    assert.are.equal("system",    body.messages[1].role)
    assert.are.equal("user",      body.messages[2].role); assert.are.equal("q1", body.messages[2].content)
    assert.are.equal("assistant", body.messages[3].role); assert.are.equal("a1", body.messages[3].content)
    assert.are.equal("user",      body.messages[4].role); assert.are.equal("q2", body.messages[4].content)
  end)
...
describe("session management", function()
  ...
  it("after reset, OpenAI first request has no previous_response_id", function()
    ...
    assert.is_table(body)
    assert.is_nil(body.previous_response_id,
      "previous_response_id must be absent after a session reset")
  end)
