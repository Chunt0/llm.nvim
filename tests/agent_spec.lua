...
describe("Agent.expand_mentions", function()
  local fixture = "/tmp/llm_nvim_mentions_fixture"
  os.execute("rm -rf " .. fixture .. " && mkdir -p " .. fixture .. "/src")
  local f = assert(io.open(fixture .. "/src/util.lua", "w"))
  f:write("return { answer = 42 }\n")
  f:close()
  local e = assert(io.open(fixture .. "/.env", "w"))
  e:write("SECRET=x\n")
  e:close()

  it("attaches a mentioned project file", function()
    local expanded, attached = Agent.expand_mentions("explain @src/util.lua please", fixture)
    assert.are.same({ "src/util.lua" }, attached)
    assert.truthy(expanded:match("# Attached files"))
    assert.truthy(expanded:match("answer = 42"))
    -- the original prompt text is preserved at the top
    assert.truthy(expanded:match("^explain @src/util%.lua please"))
  end)

  it("dedupes repeated mentions and strips trailing dots", function()
    local _, attached = Agent.expand_mentions("see @src/util.lua and @src/util.lua.", fixture)
    assert.are.same({ "src/util.lua" }, attached)
  end)

  it("ignores missing files, secrets, and escapes", function()
    local expanded, attached = Agent.expand_mentions("@nope.lua @.env @../../etc/passwd hi", fixture)
    assert.are.same({}, attached)
    assert.are.equal("@nope.lua @.env @../../etc/passwd hi", expanded)
  end)

  it("truncates huge attachments at the byte cap", function()
    local big = assert(io.open(fixture .. "/big.txt", "w"))
    big:write(string.rep("x", 500) .. "\n")
    big:close()
    local expanded = Agent.expand_mentions("@big.txt", fixture, { max_bytes = 100 })
    assert.truthy(expanded:match("%[truncated"))
  end)

  it("leaves prompts without mentions untouched", function()
    local expanded, attached = Agent.expand_mentions("no mentions here", fixture)
    assert.are.equal("no mentions here", expanded)
    assert.are.same({}, attached)
  end)
end)
