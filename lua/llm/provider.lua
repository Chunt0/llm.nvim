...
function P.create(opts)
  ...
  local function get_prompts()
    if opts.constants then
      return opts.constants.prompts
    end
    return opts.prompts or { system_prompt = "", code_prompt = "" }
  end
  local function get_vars()
    if opts.constants then
      return opts.constants.vars
    end
    return opts.vars or {}
  end
  ...
  -- Single-turn Q&A: streams an answer, keeps no conversation history.
  function M.invoke()
  ...
  -- Replace the visual selection. Streams into a scratch vsplit and applies
  -- the result only when the request completes successfully — the original
  -- buffer is never touched by a failed or cancelled request.
  function M.code()
    local p, v = get_prompts(), get_vars()
  ...
  -- Replace with review: same as code(), but the proposed change is shown as
  -- a native diff first and applied only on the accept keymap.
