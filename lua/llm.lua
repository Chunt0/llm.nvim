...
-- Updated code to create new OpenAI spec curl args and handle openai spec curl data
-- This file defines a function buildOpenAISpecCurl that constructs curl command
-- arguments for the new response endpoint and handles reasoning effort and prev_response_ids.
-- This is suppose to be written in lua. refer to my previous openai spec
-- 
-- example of new response patter:
...
-- if this is the second or greater response for openai. ensure to add previous_response_id
-- to do this you would take the resp_id from the previous response and save it. then when another chat response is generated by the user you would add it to the new curl command like this:
curl https://api.openai.com/v1/responses \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $OPENAI_API_KEY" \
  -d '{
    ...
    "previous_response_id": "resp_sdkjfhg34hrkhbre" -- the response id can be found in the previous response.s "id" field
    "stream": true
    "reasoning": {
	"effort": "minimal"
    },
...
function M.handle_openai_spec_data(line)
	...
	if delta_text == "" and json.choices and json.choices[1] and json.choices[1].delta and json.choices[1].delta.content then
